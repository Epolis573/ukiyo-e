...
export function getHelperMetadata(babel, code, helperName) {
  ...
  const validateDefaultExport = decl => {
    if (exportName) {
      throw new Error(`Helpers can have only one default export (in ${helperName})`);
    }

    if (!decl.isFunctionDeclaration() || !decl.node.id) {
      throw new Error(`Helpers can only export named function declarations (in ${helperName})`);
    }
  };
  ...
  const dependencyVisitor = {
    Program(path) {
      for (const child of path.get("body")) {
        if (child.isImportDeclaration()) {
          if (child.get("specifiers").length !== 1 || !child.get("specifiers.0").isImportDefaultSpecifier()) {
            throw new Error(`Helpers can only import a default value (in ${helperName})`);
          }
      ...
      for (const child of path.get("body")) {
        if (child.isExportDefaultDeclaration()) {
        ...
        } else if (child.isExportNamedDeclaration() && child.node.specifiers.length === 1 && child.get("specifiers.0.exported").isIdentifier({ name: "default" })) {
          ...
          spansToRemove.push([child.node.start, child.node.end]);
          child.remove();
        } else if (process.env.IS_BABEL_OLD_E2E && child.isExportNamedDeclaration() && child.node.specifiers.length === 0) {
          spansToRemove.push([child.node.start, child.node.end]);
          child.remove();
        } else if (child.isExportAllDeclaration() || child.isExportNamedDeclaration()) {
      ...
      Object.keys(bindings).forEach(name => {
        ...
        const references = [...binding.path.getBindingIdentifierPaths(true)[name].map(makePath), ...binding.referencePaths.map(makePath)];
    ...
    AssignmentExpression(child) {
      ...
      if (!left.isIdentifier()) {
        throw new Error(`Only simple assignments to exports are allowed in helpers (in ${helperName})`);
      }
